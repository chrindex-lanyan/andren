...
namespace chrindex::andren::network
{
    ...
    Http2ndSession::Http2ndSession( int endType  , SSLStream && sslstream) 
        ...
        // 异步地发送一个请求
        bool Http2ndSession::asendResponse()
        {
        ...
        {
            /**
             * 为了说清楚函数的被调用实机，现假设一种情况：
             * 一个请求由10个帧组成，其中head部分占3个帧，数据部分占7个帧。
             * 则 ： 
             *   on_frame_recv_callback： 在每个帧接收后都会被调用一次，包括 HEADER 帧和 DATA 帧。在例子中，这个回调函数会在每个帧接收时都被调用,共10次。
             ...
             *   on_stream_close_callback：当stream_id被close时会被回调1次，或者对于处于保留状态的stream也会触发此回调。
             */
        ...
        int Http2ndSession::nghttp2OnHeadDone(nghttp2_session *session, const nghttp2_frame *frame, const uint8_t *name, size_t namelen, const uint8_t *value, size_t valuelen, uint8_t flags, void *user_data)
        {
            ...
            /// 我在此处只保存reqname和reqvalue。
            /// 你当然可以在帧回调里处理所有的数据，但这样就没人帮你处理HPACK的情况了。
            /// 至于回调，因为此时还未push帧到stream里，因为我不在此处进行回调以通知用户。
            ...
            //  保存到流
            stream_data->reqname = std::move(reqname);
            stream_data->reqvalue = std::move(reqvalue);
